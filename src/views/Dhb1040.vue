<template>
   <div>
      <LayoutBar pcTitulo="ESTADÍSTICAS CONVALIDACIONES"/>
               ...
               <div v-if="!plLoading && paDatos" style="width:100%; max-height:400px; overflow-y:auto; border:1px solid #ccc;">
                  <table border="1" style="width:100%; border-collapse:collapse; font-size:14px;">
                     <thead>
                        ...
                        <th style="padding:6px; position:sticky; top:0; background:#f2f2f2; text-align:right;">AC.</th>
                        <th style="padding:6px; position:sticky; top:0; background:#f2f2f2; text-align:right;">REC.</th>
                        <th style="padding:6px; position:sticky; top:0; background:#f2f2f2; text-align:right;">TR.</th>
                     ...
                     <tbody>
                        <tr v-for="laItem in paDatos" :key="laItem.CUNIACA">
                           ...
                           <td style="padding:6px; text-align:right;" title="Aceptados">{{f_Format(laItem.NACEPTA, 0)}}</td>
                           <td style="padding:6px; text-align:right;" title="Rechazados">{{f_Format(laItem.NRECHAZ, 0)}}</td>
                           <td style="padding:6px; text-align:right;" title="En trámite">{{f_Format(laItem.NTRAMIT, 0)}}</td>
                           <td style="padding:6px; text-align:right;" title="En trámite + 10 días">{{f_Format(laItem.NTRAVEN, 0)}}</td>
   ...
   async function f_Init() {
      try {
         ...
         const response = await fetch('http://localhost:3000/', {
            ...
            body: JSON.stringify({ID: 'Dhb1040i', CCODUSU: '1221'})
         })
         if (!response.ok) {
